...
/// Simple working connection test based on commit 049ddca
/// This follows the exact pattern that worked in Phase 1
pub fn main() {
  io.println("\n=== SIMPLE WORKING CONNECTION TEST ===")
  io.println("Based on commit 049ddca (Phase 1, Step 13)")
  io.println("")
  ...
  case connection.connect(conn_config) {
    Ok(conn) -> {
      io.println("✅ Successfully connected to IB TWS API")
      io.println("")
      ...
      case connection.send_bytes(conn, handshake) {
        Ok(_) -> {
          ...
          io.println("⏳ Waiting 2 seconds for server response...")
          connection.sleep(2000)
